...
describe('Algo Trading Engine', () => {
    ...
    describe('Order Matching - Price Priority', () => {
        ...
        test('should execute trade at price of earlier order', () => {
            algo.addBuyOrder(105, 10, 'buyer1');
            algo.addSellOrder(100, 10, 'seller1');
            
            const trades = algo.getTrades();
            expect(trades[0].price).toBe(105); // Buy order was first, so use buy price
            ...
            expect(trades2[0].price).toBe(100); // Sell order was first, so use sell price
        });
    });
